...
class ReservationServiceTest {
    ...
    void setUp() throws IOException {
        ...
        // UserRepository needs a file path - point it at a temp file so it doesn't
        // touch the real users.csv
        String tempUsers = tempDir.resolve("users.csv").toString();
    ...
    void getForUserOnlyReturnsTheirReservations() {
        Equipment eq2 = new Equipment(UUID.randomUUID(), EquipmentType.SIGNAL_GENERATOR, "Signal Generator", LabLocation.LAS1001);
    ...
    @Test
    void bookedReservationHasCorrectEquipment() {
        Reservation res = service.book(student, equipment, LocalDateTime.now().plusDays(1), 2, "CERT-001", new CreditCardPayment());
        assertEquals(equipment.getEquipmentId(), res.getEquipment().getEquipmentId());
    }

    @Test
    void bookedReservationHasCorrectUser() {
        Reservation res = service.book(student, equipment, LocalDateTime.now().plusDays(1), 2, "CERT-001", new CreditCardPayment());
        assertEquals(student.getID(), res.getUser().getID());
    }

    @Test
    void bookedReservationHasCorrectHours() {
        Reservation res = service.book(student, equipment, LocalDateTime.now().plusDays(1), 4, "CERT-001", new CreditCardPayment());
        assertEquals(4, res.getHours());
    }

    @Test
    void cancelledReservationNoLongerBlocksTimeSlot() {
        LocalDateTime start = LocalDateTime.now().plusDays(1);
        Reservation first = service.book(student, equipment, start, 2, "CERT-001", new CreditCardPayment());
        service.cancel(first);

        // same slot should now be free
        assertDoesNotThrow(() ->
                service.book(faculty, equipment, start, 2, "CERT-002", new CreditCardPayment()));
    }

    @Test
    void extendIncreasesTotalCostCorrectly() {
        Reservation res = service.book(student, equipment, LocalDateTime.now().plusDays(1), 2, "CERT-001", new CreditCardPayment());
        service.payBalance(res);
        double costBefore = res.getTotalCost();
        service.extend(res, 2);
        assertEquals(costBefore + (res.getHourlyRate() * 2), res.getTotalCost());
    }

    @Test
    void cannotExtendByZeroHours() {
        Reservation res = service.book(student, equipment, LocalDateTime.now().plusDays(1), 2, "CERT-001", new CreditCardPayment());
        service.payBalance(res);
        assertThrows(IllegalArgumentException.class, () -> service.extend(res, 0));
    }

    @Test
    void cannotModifyWithZeroHours() {
        LocalDateTime start = LocalDateTime.now().plusDays(1);
        Reservation res = service.book(student, equipment, start, 2, "CERT-001", new CreditCardPayment());
        assertThrows(IllegalArgumentException.class, () ->
                service.modify(res, start.plusDays(1), 0));
    }

    @Test
    void cannotModifyWithPastStartTime() {
        LocalDateTime start = LocalDateTime.now().plusDays(1);
        Reservation res = service.book(student, equipment, start, 2, "CERT-001", new CreditCardPayment());
        assertThrows(IllegalArgumentException.class, () ->
                service.modify(res, LocalDateTime.now().minusHours(1), 2));
    }

    @Test
    void getForUserReturnsAllReservationsForThatUser() {
        Equipment eq2 = new Equipment(UUID.randomUUID(), EquipmentType.SIGNAL_GENERATOR, "Spectrometer", LabLocation.LAS1001);
        service.book(student, equipment, LocalDateTime.now().plusDays(1), 2, "CERT-001", new CreditCardPayment());
        service.book(student, eq2,       LocalDateTime.now().plusDays(3), 2, "CERT-002", new CreditCardPayment());
        service.book(faculty, equipment, LocalDateTime.now().plusDays(5), 2, "CERT-003", new CreditCardPayment());

        assertEquals(2, service.getForUser(student).size());
    }
}